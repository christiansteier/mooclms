Plugins
========

Plugins also called "Modules", Plugins contains

Plugins overview 
------------------
A list of all plugins used on the site. It also shows total number of installed plugins, disabled plugins and add-ons used on the moodle site.


Install add-ons
-----------------
To install an add-on, its source code must be put (deployed) into the appropriate location inside the Moodle installation directory and the main administration page *Administration > Site administration > Notifications* must be visited. There are three ways how the add-on code can be deployed into Moodle.

Read more about :ref:`installing_add-ons` .


Activity modules
-----------------

.. toctree::
   :maxdepth: 1
   
   manage_activities
   assignment_settings
   book_settings
   chat_settings
   database_settings
   folder_settings
   forum_settings
   glossary_settings
   ims_content_package_settings
   label_settings
   lesson_settings
   lti_settings
   page_settings
   quiz_settings
   file_settings
   scorm_settings
   url_settings
   workshop_settings
   
   
Course formats
----------------
An administrator can enable, disable or delete course formats for courses in *Administration > Site administration > Plugins > Course formats > Manage course formats*. They can also change the order in which available course formats are displayed to teachers in the course settings. 

* :ref:`Manage course format <course_format>`
* :ref:`Single activity format <single_activity_format>`
   
   
Blocks
--------
.. toctree::
   :maxdepth: 1
   
   manage_blocks
   corses
   course_overview
   html
   online_users
   remote_rss_feeds
   section_links
   tags
   
   
Message outputs
-----------------
* :ref:`Manage Message Outputs <manage_message_outputs>`
* :ref:`Default Message Outputs <default_message_outputs>`
* :ref:`Email <email>`
* :ref:`Jabber Message <jabber_message>`
  
   
Authentication
-----------------
Authentication is the process of allowing a user to login to a Moodle site based on their username and password. 

Authentication plugins
^^^^^^^^^^^^^^^^^^^^^^^^
Moodle provides a number of ways of :ref:`managing authentication <managing_authentication>`, called authentication plugins. 

.. toctree::
   :maxdepth: 1
  
   manual_accounts
   no_login
   email-based_self_registration
   mnet
   no_authentication
   other_authentication_types
   
   
Enrolments
------------
Enrolment, or 'enrollment', is the process of marking users as participants in the course, that includes both students and teachers. At the same time users are usually give a role in the course which specifies what they are allowed to do. 

Enrolment plugins
^^^^^^^^^^^^^^^^^^
Moodle provides a number of ways of :ref:`managing course enrolment <managing_enrolments>` , called enrolment plugins. Click the links below for further information. 

.. toctree::
   :maxdepth: 1

   manual_enrolments
   self_enrolments  
   cohort_syn
   guest_access
   category_enrolments
   external_database
   flat_file
   ims_enterprise
   ldap_enrolments
   course_metalink
   mnet_enrolments
   paypal
   
   
Text Editors
------------
* :ref:`Manage editors <manage_editors>`
* :ref:`TinyMCE HTML editor <tinymce_editor_settings>`
   
Licences
------------
.. toctree::
   :maxdepth: 1
 
   manage_licences


Filters
---------
.. toctree::
   :maxdepth: 1
 
   manage_filters
   filter_settings
   
   
Repositories
--------------
.. toctree::
   :maxdepth: 1
   
   manage_repositories
   common_repository_settings
   embedded_files
   server_files
   recent_files
   upload_file
   url_downloader
   private_files
   wikimedia
   youtube_videos
   

Web Services
--------------
* :ref:`Mobile <mobile_web_services>`
* :ref:`Overview <overview>`
* :ref:`API Documentation <external_services>`
* :ref:`External services <external_services>`   
* :ref:`Manage protocols <manage_protocols>`
* :ref:`Manage tokens <manage_tokens>`    

   
   
Question Behaviours
--------------------
.. toctree::
   :maxdepth: 1
   
   manage_question_behaviours
   
   
Question Types
----------------
.. toctree::
   :maxdepth: 1
   
   manage_question_types
   question_preview_defaults
   
   
Admin Tools
-------------
.. toctree::
   :maxdepth: 1
   
   manage_admin_tools
   
   


   
   
   
   



